...
const Profile = () => {
  ...
  useEffect(() => {
    // Extract the user ID from the window link
    const url = window.location.href;
    const id = url.substring(url.lastIndexOf("/") + 1);
    setUserId(id);

    // Make a GET request to fetch the user data using the extracted ID
    const fetchUser = async () => {
      try {
        const res = await axios.get(`/users/${id}`);
        // Assuming the user data is returned in the response data
        const user = res.data;
